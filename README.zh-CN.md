...
切换 Git 分支后，扩展会自动以当前工作区内容重新开始审查，清空旧的待审查变更和 History；文件内容不会被回退，旧 Timeline 仍保留。同一分支上的普通提交不会重置审查。此行为也适用于 Git worktree 和切入／切出 detached HEAD；多根工作区中任一仓库切换分支会重置整个工作区的审查。
